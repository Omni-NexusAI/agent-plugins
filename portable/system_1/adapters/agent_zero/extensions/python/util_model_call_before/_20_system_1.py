...
try:
    from usr.plugins.system_1.helpers.utility import (
        ...
        install_utility_measurement, utility_call_category)
...
class SystemOneUtility(Extension):
    async def execute(self, call_data: dict, **kwargs):
        if self.agent:
            ...
            category = utility_call_category(call_data) if utility_call_category else None
            ...
            if route.attempted:
                if install_utility_measurement:
                    install_utility_measurement(
                        self.agent, call_data, outcome="fallback", category=category)
                return
